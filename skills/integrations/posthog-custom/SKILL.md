---
name: posthog-custom
description: Custom skill to add and use PostHog in Next.js (App Router) applications. Use when needing to (1) integrate PostHog analytics, (2) set up LLM analytics for AI SDK / Anthropic generations, (3) add error tracking, or (4) implement feature flags. Covers SDK setup, user identification, event capture, exception tracking, source maps, and server/client flag evaluation. Custom curation of PostHog's official skills for a Next.js + Vercel AI SDK stack.
...
The original PostHog skills license is preserved in [LICENSE](LICENSE).
